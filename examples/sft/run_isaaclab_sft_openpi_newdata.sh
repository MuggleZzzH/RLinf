...
CONFIG_NAME="${CONFIG_NAME:-isaaclab_sft_openpi_newdata}"
RUN_NAME="${RUN_NAME:-isaaclab_sft_openpi_newdata}"
RESULT_ROOT="${RESULT_ROOT:-${REPO_PATH}/result/isaaclab_openpi_newdata/sft}"
...
if [ -z "${OPENPI_MODEL_DIR}" ]; then
    ...
    echo "Example:"
    echo "  OPENPI_MODEL_DIR=/path/to/openpi_model_dir bash examples/sft/run_isaaclab_sft_openpi_newdata.sh"
    exit 1
fi
