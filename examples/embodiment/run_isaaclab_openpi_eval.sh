...
RUN_NAME="${RUN_NAME:-isaaclab_openpi_eval}"
RESULT_ROOT="${RESULT_ROOT:-${REPO_PATH}/result/isaaclab_openpi/eval}"
...
if [ -z "${OPENPI_MODEL_DIR}" ]; then
    ...
    echo "Example:"
    echo "  OPENPI_MODEL_DIR=/path/to/openpi_model_dir bash examples/embodiment/run_isaaclab_openpi_eval.sh"
    exit 1
fi
