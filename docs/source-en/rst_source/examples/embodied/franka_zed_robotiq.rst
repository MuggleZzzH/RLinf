...
hardware-specific installation, configuration, and a ready-made dummy config
...
A ready-made dummy configuration is provided for verifying the cluster
setup **without** a real robot or real cameras:
...
This config sets ``is_dummy: True`` for both train and eval environments, so
no actual hardware connection is attempted.  It uses the same algorithm and
model settings as the standard Franka CNN dummy config, but with
``camera_type: zed`` and ``gripper_type: robotiq`` to exercise the backend
