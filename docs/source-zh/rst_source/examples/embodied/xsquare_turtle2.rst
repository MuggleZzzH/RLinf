...
``Turtle2DeployEnv-v1``，并在 ``Turtle2Env`` 内解析 ``action_mode``。
...
- ``absolute_pose`` 直接进入 ``Turtle2Env.step_absolute_pose``。策略为每条激活机械臂
  输出一个 7 维绝对位姿动作：``[x, y, z, roll, pitch, yaw, gripper]``。
- ``relative_pose`` 是默认模式。启用 ``use_relative_frame`` 时，本地 stack 会挂载
  ``DualRelativeFrame``，将末端坐标系下的增量动作转换回机器人基坐标系。
- ``use_master_takeover=True`` 时，会在 Turtle2 stack 上额外挂载
  ``MasterTakeoverIntervention``，以及按配置启用 ``KeyboardRunningModeWrapper``。
- 两种模式最后都会经过 ``DualQuat2EulerWrapper``，因此策略看到的是欧拉角格式
  的 TCP 状态。

``apply_dual_arm_wrappers`` 仍然只负责通用双臂环境，不承载 Turtle2 特例。
