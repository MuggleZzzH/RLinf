...
注册名为 ``Turtle2DeployEnv-v1``。本 PR 新增的内容只是一个 env fragment：

.. code-block:: text

   examples/embodiment/config/env/realworld_turtle2_deploy.yaml

部署场景因 policy / 算法 / checkpoint 不同而差异极大，本 PR **不**自带
一份完整 eval 顶层配置。建议在你现有的 eval 配置（例如基于
``examples/embodiment/config/realworld_eval.yaml``）上集成这个 fragment：
替换 env default，并对齐 state / action 维度。

.. code-block:: yaml

   defaults:
     # ... 你已有的 model / training_backend / weight_syncer defaults ...
     - env/realworld_turtle2_deploy@env.eval

   actor:
     model:
       # 扁平 obs 字典序：gripper(2) + tcp_pose(12) = 14。
       state_dim: 14
       action_dim: 14
...
**本地 observation / action 合同**（仅 Turtle2 deploy 适用，**不是**仓库
全局约定）：

* env 原始输出 ``obs.state.tcp_pose`` 为 ``(14,)`` 双臂 ``xyz + quat``（base
  frame），经 ``DualQuat2EulerWrapper`` 后变为 ``(12,)`` ``xyz + rpy``。
* ``obs.state.gripper`` 为 ``(2,)`` 双臂 gripper 宽度，所有 wrapper 都不动
  这个 key（``tcp_pose`` 永远不包含 gripper 通道）。
* ``RealWorldEnv._wrap_obs`` 按 key 字典序拼接 state，所以扁平
  ``obs["states"]`` 的顺序是 ``gripper(2) + tcp_pose(12) = 14``。
  ``actor.model.state_dim`` 设为 14，policy 侧按这个顺序拆分。
* 两种模式 action 都是 ``(14,)``：

  - ``relative_pose``（默认）：每臂 ``[dx, dy, dz, drx, dry, drz, dg]``，
    pose 部分位于 EE frame；共享的 ``DualRelativeFrame`` wrapper 会在 env
    执行前把 6D 运动转回 base frame，与训练侧合同一致。
  - ``absolute_pose``：每臂 ``[x, y, z, rx, ry, rz, g]``，base frame 绝对
    目标，由 smooth controller 直接执行。该模式下 wrapper 链 **不挂**
    ``DualRelativeFrame``；但 ``DualQuat2EulerWrapper`` 仍然挂，仅用于将
    观测的 quat 转 euler（它是 observation wrapper，不修改 action）。

只有当策略输出绝对位姿命令时，才设置 ``override_cfg.action_mode:
absolute_pose``；否则保留默认 ``relative_pose``。配置中 ``action_mode``
只应写在 ``override_cfg`` 下——factory 不读取顶层 ``action_mode``。

.. note::

   在 Turtle2 控制节点上，推荐采用 **双容器分工** 来降低 ROS 冲突风险：

   - 控制容器（例如厂商提供的 Turtle2 控制容器，如 ``turtle2_release``）
     负责 ROS master、UI、底层 bring-up，以及相机/机械臂控制节点。
   - 独立的 RLinf 容器只负责 RLinf 运行时与 Ray worker，不直接接管
     ``roscore``、UI 或 ``run.sh``。

   建议 RLinf 容器基于与控制容器 **兼容的同源镜像配置** 构建，复用相同的
   基础镜像、系统依赖、ROS 运行时和必要挂载，只在其上额外加入 RLinf 所需
   的 Python 环境与代码。一个常见做法是让两个容器都使用 ``host network``，
   共享控制节点的网络栈；RLinf 容器可以直接连接已有的 ROS master，不需要
   在自己的容器里重新启动一套控制面。训练或评估时，建议仅在控制容器中临时
   打开 UI 做校准，完成后关闭 UI，再由 RLinf 容器执行任务，以避免多个进程
   同时占用机械臂控制话题。
