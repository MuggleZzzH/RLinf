...
本文档给出在 RLinf 框架内，基于 **XSquare X1** 双臂机器人平台启动真机强化学习训练任务的完整指南。
...
- **机器人**：XSquare X1 双臂桌面机器人，支持最多 2 条机械臂（左臂 ID ``0``，右臂 ID ``1``）以及最多 3 个 RGB 相机（ID ``0``、``1``、``2``）。
...
XSquare X1平台自带SDK和基于ROS的控制器。**请在开始下安装之以前，确保您已进入Xsquare的官方Docker容器**。请联系`XSquare <https://x2robot.com>`_获取准确的Docker镜像和启动说明。
...
.. note::
   ...
   在 X1 控制节点上，推荐采用**双 Docker 分工**来降低 ROS 冲突风险：

   - 控制容器（例如厂商提供的 X1 控制容器，如 ``x1_release``）负责 ROS master、UI、底层 bring-up，以及相机/机械臂控制节点。
...
根据实际设置修改配置文件 ``examples/embodiment/config/realworld_button_x1_sac_cnn.yaml``。
