...
class TrajectoryReplayBuffer:
    ...
    def _flatten_trajectory(self, trajectory: Trajectory) -> dict:
        ...
        if trajectory.curr_obs:
            curr_obs = self._flatten_nested_tensor_dict(trajectory.curr_obs)
            if curr_obs:
                flat["curr_obs"] = curr_obs

        if trajectory.next_obs:
            next_obs = self._flatten_nested_tensor_dict(trajectory.next_obs)
            if next_obs:
                flat["next_obs"] = next_obs

        if trajectory.forward_inputs:
            forward_inputs = self._flatten_nested_tensor_dict(
                trajectory.forward_inputs
            )
            if forward_inputs:
                flat["forward_inputs"] = forward_inputs

        return flat

    def _flatten_nested_tensor_dict(self, data: dict) -> dict:
        flat: dict[str, object] = {}
        for key, value in data.items():
            if isinstance(value, torch.Tensor) and value.dim() >= 2:
                flat[key] = value.reshape(-1, *value.shape[2:])
            elif isinstance(value, dict):
                nested = self._flatten_nested_tensor_dict(value)
                if nested:
                    flat[key] = nested
        return flat
    ...
    def _extract_chunk_from_flat_trajectory(
        ...
        return self._extract_index_from_nested(flat_trajectory, idx)

    def _extract_index_from_nested(self, data: dict, idx: int) -> dict:
        chunk: dict[str, object] = {}
        for key, value in data.items():
            if isinstance(value, torch.Tensor):
                chunk[key] = value[idx]
            elif isinstance(value, dict):
                nested = self._extract_index_from_nested(value, idx)
                if nested:
                    chunk[key] = nested
        return chunk

    def _init_batch_from_flat(self, flat_trajectory: dict, batch_size: int) -> dict:
        return self._init_batch_from_nested(flat_trajectory, batch_size)

    def _init_batch_from_buffer(self, buffer: dict, batch_size: int) -> dict:
        return self._init_batch_from_nested(buffer, batch_size)

    def _init_batch_from_nested(self, data: dict, batch_size: int) -> dict:
        ...
        for key, value in data.items():
            if isinstance(value, torch.Tensor):
                ...
                nested_batch = self._init_batch_from_nested(value, batch_size)
                if nested_batch:
    ...
    def _merge_chunks_to_batch(self, chunks: list[dict]) -> dict[str, torch.Tensor]:
        ...
        return self._merge_nested_chunks(chunks)

    def _merge_nested_chunks(self, chunks: list[dict]) -> dict:
        batch: dict[str, object] = {}
        all_keys = set()
        for chunk in chunks:
        ...
        for key in all_keys:
            ...
            if isinstance(first_value, torch.Tensor):
                ...
                if nested_values:
                    nested_batch = self._merge_nested_chunks(nested_values)
                    if nested_batch:
                        batch[key] = nested_batch
