...
def _apply_keyboard_reward(env: gym.Env, mode: Optional[str]) -> gym.Env:
    ...
    if getattr(getattr(base_env, "config", None), "is_dummy", False) or not mode:
        return env
    if mode == "multi_stage":
...
def _apply_keyboard_running_mode(
    env: gym.Env, cfg: Optional[Mapping[str, Any]]
) -> gym.Env:
    if cfg is None or not bool(cfg.get("enabled", False)):
        return env
    return KeyboardRunningModeWrapper(env, config=cfg)
