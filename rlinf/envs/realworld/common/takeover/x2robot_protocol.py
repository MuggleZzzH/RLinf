...
class X2RobotTakeoverTCPConfig:
    ...
    @classmethod
    def from_dict(cls, cfg: dict | None) -> "X2RobotTakeoverTCPConfig":
        if cfg is None:
            return cls()
        return cls(**cfg)
...
def recv_frame(sock: socket.socket) -> dict[str, dict | None]:
...
def send_frame(sock: socket.socket, data: dict[str, dict]) -> None:
    ...
    if msg_type == MSG_MODE:
        ...
        joint_count = int(payload.get("joint_count", min(len(joint_left), len(joint_right))))
        if joint_count <= 0:
...
class X2RobotTakeoverTCPServer:
    """Env-side TCP bridge that preserves x2robot master/slave takeover semantics."""
    ...
    def poll(self) -> None:
        ...
        with self._state_lock:
            if running_mode != self._current_mode:
                ...
                self._min_pose_timestamp_us = 0
                if running_mode == self.config.takeover_mode_value:
    ...
    def get_takeover_action(self) -> np.ndarray | None:
        with self._state_lock:
            if self._current_mode != self.config.takeover_mode_value:
                return None
            if self._follow_start_time is None or time.time() < self._follow_start_time:
                return None
            if (
    ...
    def _io_loop(self) -> None:
        while not self._stop_event.is_set():
            ...
            with self._state_lock:https://github.com/MuggleZzzH/RLinf.git
