...
class Turtle2Env(gym.Env):
    ...
    def _normalize_config_arrays(self) -> None:
        self.config.target_ee_pose = np.asarray(
            self.config.target_ee_pose, dtype=np.float64
        ).reshape(NUM_ARMS, POSE_DIM)
        self.config.reset_ee_pose = np.asarray(
            self.config.reset_ee_pose, dtype=np.float64
        ).reshape(NUM_ARMS, POSE_DIM)
        self.config.ee_pose_limit_min = np.asarray(
            self.config.ee_pose_limit_min, dtype=np.float64
        ).reshape(NUM_ARMS, POSE_DIM)
        self.config.ee_pose_limit_max = np.asarray(
            self.config.ee_pose_limit_max, dtype=np.float64
        ).reshape(NUM_ARMS, POSE_DIM)
